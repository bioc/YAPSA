---
title: "Vignette Indel Signatures"
author: "Lea Jopp-Saile and Daniel Huebschmann"
date: "1/28/2019"
vignette: >
  %\VignetteIndexEntry{Vignette Indel Signatures}
...
For INDEL signatures classification is more complex as the context of the
variant is determined by the INDEL size, which can be variable as well. In the
...
nucleotide(s).
2. Insertion of 1 bp C/(G) or T/(A) in a repetitive
...
3. Deletions of 2bps, 3bps, 4bps or more or equal to 5bps in a repetitive 
context. Each deletion is classified in a context of 1, 2, 3, 4, 5 or larger
or equal to 6 times the same motif.
4. Insertion of 2 bps, 3 bps, 4 bps or more
...
5. Microhomology deletion of 2bps, 3bps, 4bps or more or equal to 5 bps in a
...
INDEL classifications in the range of 1 bp till 5 bp considered
individually, while all variant larger than 5bp are considered as one feature.
In this context, microhomology is defined by the partial sequence similarity
between the motif of the INDEL and the immediate sequence context. In order to
...
With the function `plotExchangeSpectra_indel` one can plot the nuceleotide
...
cap <-": Nucleotide exchange profile of INDEL signature ID3 imprinted by
        tobacco smoking and ID6 related to deficient homologous recombination 
        repair pathway. Figure was created with YAPSA function
...
INDEL signatures. Signatures 15 was excluded as none of the samples had a
...
The data set is not related to cancer genomes but instead contains germ line
variation within the genomes of Dutch people. Hence an interpretation of the
data in terms of mutational process is not an appropriate conclusion, which can
...
data frame with `r dim(GenomeOfNl_raw)[1]` variants.
...
The obtained data frame is in a variant calling format, which will be used to
...
in the different samples. To do so, from the `GenomeOfNl_raw` data frame 15
...
kable(head(vcf_like_indel_df), caption="Head of the vcf_like_df
      containing the GoNL INDEL data, which is used for to
      build $V^{INDEL}$")
```

The artificial created `vcf_like_df` file is than be used as in input for the
...
2. `attribution_of_idels` attributes each variant to one of the 83 classes of
features 
3. `create_indel_mut_cat_from_df`rearranges the data frame to obtain a the
...
applied to the INDEL mutational catalog to perform INDEL supervised mutational
...
YAPSA has signature-specific cutoffs integrated for PCAWG SNV and INDEL
...
stored in data frames within the software package and can be loaded into the
work-space.

```{r load_cutoffs, warning=FALSE}
data(cutoffs_pcawg)
```
In order to identify the optimal INDEL signature-specific cutoffs, the minimal
false attribution represented bu the cost factor is required. For INDEL
signature an optimal cost factor of 3 was be identified, equivalent to the third
line in `cutoffPCAWG_ID_WGS_Pid_df`.

Now all required information are provided to perform the INDEL mutational
signature decomposition, except for the data frame `PCAWG_SP_ID_sigInd_df` which
is loaded with the mutational signature data frames and contains the mutational
...
cap <- ":Exposures to mutational signatures of the artificial created INDEL
        ...
        a decompostion with PCAWG INDEL sigantures as well as their signature
