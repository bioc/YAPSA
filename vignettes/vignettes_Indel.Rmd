...
lengthm with microhomology, i.e., partially repetitive context of 4 bp at the 
...
We first load the example data, which is stored in the the software package 
...
vcf-like dataframe with `r dim(GenomeOfNl_raw)[1]` variants.
...
The obtained dataframe corresponds to a modified variant calling format (vcf). 
We will later use it in order to build a mutational catalog for supervised 
analysis of mutational signatures with PCAWG Indel signatures.
...
The `GenomeOfNl_raw` has no patient identifier information annotated. However, 
...
patient identifiers (PIDs) were generated and 50 +/- 20 Indels were random 
sampled from the `GenomeOfNl_raw` dataframe per PID.
...
the deconvolution is based on a non-negative least squares algorithm. In order 
to reduce false positive calls, 
...
the calculation of confidence intervals (CIs). Analogously to CIs for SNV 
mutational signatures, the CIs for Indel mutational signatures are computed 
using the concept of profile likelihood.
